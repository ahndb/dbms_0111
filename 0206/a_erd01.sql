...
# 2. 의사소통 개선
# : 개발자, 비즈니스 사용자 간의 의사소통 도구의 역활
# : 시각적 표현으로써 프로젝트 참여자의 명확한 의사소통을 촉진

#! ERD 규칙

# A는 부모, B는 자식의 관계를 가진 ERD가 존재할 때
# 하나의 A는 하나의 B로 구성되어야 한다
# 하나의 A는 하나의 B를 포함해야하는 것.

# 실선
# : 부모의 기본키를 자식이 가지고 있고
# : 이를 기본키로 사용하는 경우(PK사용)

# 점선
# : 부모의 기본키를 자식이 가지고 있지만
# : 이를 기본키로 사용하지 않는 경우(pk미사용)

# 학생(부모)테이블 - 수강내역(자식)테이블
# 부모 테이블의 PK를 자식테이블에서 PK로 사용
# 1명의 학생에게 0 ~ N 개의 수강 내역이 존재할 수 있다.
