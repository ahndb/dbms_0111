...
# 프로시져 내에 position 변수를 선언
# 해당 포지션에 players 테이블에서 player_id를 가져옴
# if  조건문으로 포지션이 '타자'인 경우 출력문 A
# 그렇지 않은 경우 출력문 B
...
begin
	declare v_position_position varchar(100);
    select position into v_position_position
	from players
	where player_id = p_player_id;
	
    if v_position_position = '타자' then
