...
# 지역별 평균 포인트 계산 / 평균 포인트가 200 이상인 지역만 조회
select
...
# 3번째 부터 2개의 행 조회
select * from `member`order by name limit 2 offset 2;

# 포인트가 가장 높은 상위 3명의 회원 조회
...
select area_code, max(points) from `member` group by area_code;

# 성별(gender)에 따른 평균 포인트 조회
select gender, max(points) from `member` group by gender;

# 평균 포인트가 250 이상인 등급만 조회
select	grade, avg(points)
from `member`
group by grade
having avg(points) >= 250;

# 등급별 최소 포인트가 100이상인 그룹 조회
