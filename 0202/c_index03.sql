...
# create [unique] index 인덱스_이름  -- unique 미입력시 중복 가능해짐
...
# >> altrt table 문으로 기본 키나 고유 키를 제거하면
#	: 자동으로 생성된 인덱스도 제거 가능
...
show table status;
# >> Date_length: 클러스터형 인덱스의 크기를 Byte로 표기
# MySQL의 1페이지 크기는 기본적으로 16KB(16384)

# >> Index_length: 보조 인덱스의 크기
# member 테이블은 보조 인덱스가 없기 때문에 표기 되지 않음
...
# >> Executeion Plan 창에서 Full Table Scan을 제외한 결과는
#	, 모두 인덱스를 사용했다는 의미
...
# Full Table Scan
# where문에서 열에 연산을 가하는 경우 인덱스 사용 안함

select mem_name, mem_number
from member
where mem_number >= 14 / 2;
# Index Range Scan
# >> where 절에서는 열이름에 연산을 하지 말아야 함

#! 인덱스 제거
show index from member;
# idx_member_mem_name, idx_member_addr, idx_member_mem_number

# 인덱스 제거 시
# : 클러스터형 인덱스와 보조 인덱스가 섞여 있을 경우
# : 보조 인덱스를 먼저 제거하는 것을 권장
# (보조 인덱스 사이의 순서는 상관 없음)
# drop index 인덱스명 on table명;
drop index idx_member_mem_name on member;
drop index idx_member_addr on member;
drop index idx_member_mem_number on member;

# > 클러스터형 인덱스를 먼저 제거해도 가능
# 	, 클러스터형의 경우 데이터를 재구성하기 때문에 시간 소요 증가함

# 기본 키 지정으로 자동 생성된 클러스터형 인덱스 제거
# >> drop index문으로 제거 안됨 / alter table문으로만 제거 가능

alter table member
	drop primary key;
# member의 mem_id 열을 buy 참고하기 때문에 오류 남
# >> 기본키 제거 시 반드시 외래 키와의 관계를 제거

# 외래키 제거 방법 
# : 외래키의 명을 확인
# >> information_schema 데이터베이스의
# >> , referential_constraints 테이블을 조회하면 외래키명 확인
