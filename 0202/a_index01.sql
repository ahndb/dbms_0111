...
#! 인덱스 종류
# 클러스터형 인덱스
# : 기본 키(PK)로 지정하면 자동 생성, 테이블 당 1개만 생성 가능
# : 기본 키로 지정한 열을 기준으로 자동 정렬
# : 중복 안됨, null 값 안됨


# 보조 인덱스
# : 고유 키(UNIQUE)로 지정하면 자동 생성, 테이블 당 여러 개 생성 가능
# : 자동 정렬 X 
# : 중복 안됨, null 값 가능
...
# : 속도 향상 시 > 컴퓨터의 처리 속도가 빨라짐 (전체 시스템 성능 향상)

# 2. 단점
# : 인덱스도 공간을 차지하기 때문에 
#	데이터베이스 안에 추가적인 공간이 필요 (테이블 크기의 10%)
# : 처음에 인덱스를 만드는데 시간이 소요

#! 인덱스의 종류
# 1. 클러스터형 인덱스(ex. 영어사진, 국어사전)
...
# member 테이블은 회원 아이디(mem_id)가 기본 키로 설정되어 있음
