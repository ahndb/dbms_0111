...
# describe table_name;
describe `students`;
# Field(각 컬럼의 이름), Type(각 컬럼의 데이터 타입), Null(Null의 허용 여부)
...
# drop문은 해당 삭제하고자하는 형태가 존재하지 않을 경우 오류발생
# : if exists 사용 시 존재할 때만 삭제하기 때문에 오류방지 가능

drop table if exists `lectures`; -- 오류 발생 시 해당 위치부터 그 아래의 코드 실행이 안됨
drop table if exists `students`;
drop table if exists `integer`;

# 5. 테이블 목록 조회(show tables)
# : 데이터베이스에 있는 모든 테이블의 목록을 조회
