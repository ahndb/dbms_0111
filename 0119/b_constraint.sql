...
# : 외래 키가 설정된 열은 반드시 다른 테이블의 기본 키와 연결 됨
...
-- insert into `Orders`
-- values (1, "2024-01-19", 1); -- Error(기본테이블 값 누락)

# 외래 키 제약조건 삭제 & 추가
# : 외래 키 제약조건 삭제 주의점
# : 해당 데이터 존재 시 삭제 x
-- alter table `Orders`
-- drop foreign key customer_id; -- 삭제시

-- alter table `Orders`
-- add constraint
-- foreign key (customer_id) 
-- references `Customers`(customer_id); -- 추가시
...
-- values (1, null, "qwe123"); -- Error(null 값 허용X)
